...
public class AssignEmployee extends JFrame {
	...
	public AssignEmployee(String number) {
        this.number = number;
		...
		JLabel lblNewUserRegister = new JLabel("Assign Employee");
		...
		// Job ID Label
		JLabel lblName = new JLabel("Job ID");
		...
		jobID.setFont(new Font("Segoe UI", Font.PLAIN, 20));
		jobID.setBounds(210, 200, 250, 40);  // Changed Y to 200 (same row as Job ID label)
		styleTextField(jobID, fieldBg, fieldText);
		contentPane.add(jobID);

		
		// Assign Button
		btnNewButton = new JButton("Assign");
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String job_id = jobID.getText().trim();
				...
				try {
					...
					if (x > 0) {
						JOptionPane.showMessageDialog(btnNewButton,
								"Employee " + number + " successfully assigned to Job " + job_id,
								"Success",
